...
export default class VariationSelector extends Component {
	...
	componentWillReceiveProps(props) {
		...
		let defaultVariationName =
			findKey(props.font.namedVariations, n =>
				isEqual(props.font.namedVariations[n], defaultSettings)
			) ||
			...
			<div className="variation-selector">
				{Object.keys(this.props.font.namedVariations).length > 0 && (
					<select onChange={this.onChange}>
						{Object.keys(this.props.font.namedVariations).map(
							name => (
								<option
									selected={name === this.state.variationName}
								>
									{name}
								</option>
							)
						)}
						{!this.props.font.namedVariations[
							this.state.variationName
						] && (
							<option selected disabled>
								{this.state.variationName || 'Custom'}
							</option>
						)}
					</select>
				)}
				...
				{this.props.children.map(c =>
					cloneElement(c, { font: this.state.font })
				)}
			</div>
		);
	}
